...
//
pub fn model_lanternfish_growth_naive() {
    let lanternfish = (0..80).fold(LANTERNFISH.to_vec(), |curr_gen, _| {
        curr_gen.into_iter().fold(vec![], |mut next_gen, fish| {
            if fish == 0 {
                next_gen.extend_from_slice(&[6, 8]);
            } else {
                next_gen.extend_from_slice(&[fish - 1]);
            }
            next_gen
        })
    });
...
fn get_lanternfish_offspring(fish: u8, days_left: u16) -> u64 {
    if days_left < fish as u16 {
        return 0;
    }
    let a = (0..days_left)
        .rev()
        .fold((0, fish), |(total_offspring, fish), curr_days_left| {
            if fish == 0 {
                let new_fishes = get_lanternfish_offspring(8, curr_days_left) + 1;
                (total_offspring + new_fishes, 6)
            } else {
                (total_offspring, fish - 1)
            }
        });
    a.0
}
pub fn model_lanternfish_growth() {
    let now = std::time::Instant::now();
    let num_days = 256;
    let total_offspring = [1, 2, 3, 4, 5]
        .into_iter()
        .map(|fish| get_lanternfish_offspring(fish, num_days));
    ...
    let b = total_offspring
        .zip(starting_days_count)
        .fold(0u64, |acc, (offspring, starting_fish)| {
            acc + (starting_fish * offspring)
        });
...
const ALANTERNFISH: [u8; 5] = [3, 4, 3, 1, 2];

const LANTERNFISH: [u8; 300] = [
