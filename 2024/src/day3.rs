 ...
 * */
pub fn get_valid_memory() -> i64 {
    let corrupted_program = &read_input("./inputs/day3.txt").join("");
    let valid_mul_regex = Regex::new(r"mul\((\d{1,3}),(\d{1,3})\)").unwrap();
    let mul_pairs: Vec<(i64, i64)> = valid_mul_regex
        .captures_iter(&corrupted_program)
        .map(|caps| {
...
pub fn get_enabled_muls() -> i64 {
    let corrupted_program = &read_input("./inputs/day3.txt").join("");
    let disabled_muls_re = Regex::new(r"don't\(\).*?do\(\)|don't\(\).*?$").unwrap();
    let valid_mul_regex = Regex::new(r"mul\((\d{1,3}),(\d{1,3})\)").unwrap();
