...
pub fn collate_customs_answers() -> usize {
    QUESTIONAIRES.split("\n\n").fold(0, |acc, question_batch| {
        let q: HashMap<_, _> = question_batch
            .lines()
            .flat_map(|answers| answers.chars().map(|c| (c, true)))
            .collect();
        acc + q.len()
    })
}
...
pub fn collate_customs_answers_correctly() -> usize {
    QUESTIONAIRES.split("\n\n").fold(0, |acc, question_batch| {
        let len = question_batch.lines().count();
        let q: HashMap<_, _> = question_batch
            .lines()
        ...
        let total = q.iter().fold(0, |total, (c, count)| {
            if *count == len {
                return total + 1;
            }
            total
...
const QUESTIONAIRES: &str = "wdcmlzfnugqtvjbsahi
easrkmocxbpjgi
