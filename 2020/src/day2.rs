...
pub fn find_valid_passwords() -> usize {
    PASSWORDS
        .iter()
        .fold(0, |mut total_valid, (min, max, c, password)| {
            let password_chars: HashMap<char, usize> =
                password.chars().fold(HashMap::new(), |mut map, curr| {
                    let count = map.entry(curr).or_insert(0);
                    *count += 1;

                    map
                });
            let required_chars = password_chars.get(c).unwrap_or(&0);
            if (min..=max).contains(&required_chars) {
                total_valid += 1;
            }
            total_valid
        })
...
pub fn find_valid_passwords_positional() -> u32 {
    PASSWORDS.iter().fold(
        0,
        |mut total_valid, (first_pos, second_pos, c, password)| {
            let password_chars: Vec<char> = password.chars().collect();
            if !(password_chars[first_pos - 1] == *c && password_chars[second_pos - 1] == *c)
                && (password_chars[first_pos - 1] == *c || password_chars[second_pos - 1] == *c)
            {
                total_valid += 1;
            }
            total_valid
...
const PASSWORDS: [(usize, usize, char, &str); 1000] = [
    (2, 6, 'w', "wkwwwfwwpvw"),
