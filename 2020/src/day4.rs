...
impl Passport {
    ...
    fn is_valid_height(&self) -> bool {
        ...
        let hgt = self.hgt.as_ref().cloned().unwrap();

        let height: String = hgt.chars().take_while(|c| c.is_numeric()).collect();
        ...
        let valid_cm = unit == "cm" && (150..=193).contains(&height);
        let valid_in = unit == "in" && (59..=76).contains(&height);
    ...
    fn is_valid_hair_color(&self) -> bool {
        ...
        let hcl = self.hcl.as_ref().cloned().unwrap();
        let mut chars = hcl.chars();
        let first_char = chars.next() == Some('#');
        let hex_code = chars.all(|c| c.is_numeric() || ('a'..='f').contains(&c));
        first_char && hex_code
    }
    ...
    fn is_valid_pid(&self) -> bool {
        ...
        let pid = self.pid.as_ref().cloned().unwrap();
    ...
    pub fn is_valid(&self) -> bool {
        ...
        && (1920u16..=2002).contains(&self.byr.unwrap())
        //iyr (Issue Year) - four digits; at least 2010 and at most 2020.
        && (2010u16..=2020).contains(&self.iyr.unwrap())
        //eyr (Expiration Year) - four digits; at least 2020 and at most 2030.
        && (2020u16..=2030).contains(&self.eyr.unwrap())
        && self.is_valid_pid()
        && self.is_valid_height()
        && self.is_valid_hair_color()
        && self.ecl != Some(EyeColor::Invalid)
    }
...
fn parse_passports(raw_passports: Vec<String>) -> Vec<Passport> {
    raw_passports
        .iter()
        .fold(Vec::new(), |mut valid_passports, passport| {
            let passport =
                passport
                    .split_whitespace()
                    .fold(Passport::new(), |mut partial_passport, field| {
                        let mut t = field.split(':').take(2);
                        let name = t.next().unwrap();
                        let value = t.next().unwrap();
                        match name {
                            "byr" => {
                                let byr = value.parse::<u16>();
                                partial_passport.byr =
                                    if let Ok(byr) = byr { Some(byr) } else { None }
                            }
                            "cid" => partial_passport.cid = Some(value.to_owned()),
                            "ecl" => {
                                partial_passport.ecl = match value {
                                    ...
                                    _ => Some(EyeColor::Invalid),
                                }
                            }
                            "eyr" => {
                                let eyr = value.parse::<u16>();
                                partial_passport.eyr =
                                    if let Ok(eyr) = eyr { Some(eyr) } else { None }
                            }
                            "hcl" => partial_passport.hcl = Some(value.to_owned()),
                            "hgt" => partial_passport.hgt = Some(value.to_owned()),
                            "iyr" => {
                                let iyr = value.parse::<u16>();
                                partial_passport.iyr =
                                    if let Ok(iyr) = iyr { Some(iyr) } else { None }
                            }
                            "pid" => partial_passport.pid = Some(value.to_owned()),
                            _ => (),
                        };
                        partial_passport
                    });
...
pub fn validate_passports() -> usize {
    let raw_passports =
        PASSPORTS
            .lines()
            .skip(1)
            .fold(vec![String::new()], |mut passports, line| {
                if line.is_empty() {
    ...
    let valid_passports = parse_passports(raw_passports);
    valid_passports.len()
}
...
pub fn strict_validate_passports() -> usize {
    let raw_passports =
        PASSPORTS
            .lines()
            .skip(1)
    ...
    let valid_passports = parse_passports(raw_passports);
    let valid_passports: Vec<_> = valid_passports
        .iter()
